...
import { truncateMessagesSecure, assertCostLimit, recordSuccess, recordFailure, ResourceExhaustionError, DEFAULT_LIMITS } from '@/lib/security/resource-guard'
...
${messageHistoryText}`
      ...
      return validation.persona as unknown as PersonaProfile
    } catch (error: unknown) {
      if (error instanceof ResourceExhaustionError) recordFailure(circuitKey)
      const msg = error instanceof Error ? error.message : String(error)
      console.error('[WhatsappPersonaLearner] Falha ao aprender persona:', msg)
      return DEFAULT_PERSONA
    }
  }
}
