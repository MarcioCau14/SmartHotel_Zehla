...
export function DashboardPreviewSection() {
  ...
  const accentBg = isPousadas ? 'bg-emerald-500/10' : 'bg-blue-500/10';
  const accentBorder = isPousadas ? 'border-emerald-500/20' : 'border-blue-500/20';
  const accentText = isPousadas ? 'text-emerald-400' : 'text-blue-400';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <div className="pt-6">
              <Link
                href="#precos"
                className={`inline-flex items-center gap-2 px-6 py-3 ${isPousadas ? 'bg-emerald-500 hover:bg-emerald-600' : 'bg-blue-500 hover:bg-blue-600'} text-white font-bold rounded-xl transition-all duration-200 shadow-lg ${isPousadas ? 'shadow-emerald-500/10' : 'shadow-blue-500/10'} text-sm sm:text-base cursor-pointer hover:translate-x-1 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ${isPousadas ? 'focus-visible:outline-emerald-500' : 'focus-visible:outline-blue-500'}`}
              >
              ...
              >
                ...
                <div className="bg-[#16161a] border-b border-white/[0.05] px-4 py-3 flex items-center justify-between shrink-0">
                  ...
                  <div className={`flex items-center gap-1.5 ${accentText} font-medium text-[10px]`}>
                    <span className={`w-1.5 h-1.5 rounded-full ${isPousadas ? 'bg-emerald-500' : 'bg-blue-500'} animate-ping`} />
                    Conectado
                  </div>
                </div>
                    ...
                    {dashboard.chatConversation.map((msg, i) => (
                      <div key={i} className={`p-2.5 ${i === 0 ? 'bg-white/[0.02]' : ''} border-b border-white/[0.03] flex flex-col gap-1`}>
                        <div className="flex justify-between items-center">
                          ...
                          {msg.sender === 'bot' ? (
                            <span className={`text-[7px] px-1 py-0.5 ${isPousadas ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-blue-500/10 border-blue-500/20 text-blue-400'} border rounded-sm`}>IA ATIVA</span>
                          ) : (
                  ...
                  <div className="flex-1 bg-[#16161a] flex flex-col justify-between">
                        ...
                        msg.sender === 'user' ? (
                          ...
                          <div key={i} className="flex flex-col space-y-1 items-end max-w-[85%] ml-auto">
                            <span className={`text-[8px] ${accentText} mr-1 font-semibold flex items-center gap-1`}>
                              ZÉLLA (IA)
                              {msg.confidence && (
                                <span className={`px-1 py-[0.5px] ${isPousadas ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-blue-500/20 border-blue-500/30 text-blue-400'} border text-[6px] font-bold rounded-sm`}>
                                  {msg.confidence} Confiança
                                </span>
                              )}
                            </span>
                            <div className={`${isPousadas ? 'bg-emerald-950/40 border-emerald-500/20 text-emerald-300' : 'bg-blue-950/40 border-blue-500/20 text-blue-300'} border p-2.5 rounded-2xl rounded-tr-sm text-[11px] leading-relaxed`}>
                              {msg.text}
                            </div>
                            {msg.actions && (
                              <span className={`text-[7px] ${isPousadas ? 'text-emerald-500/70' : 'text-blue-500/70'} font-semibold tracking-wide mt-0.5`}>
                    ...
                    <div className="p-3 bg-[#111114] border-t border-white/[0.04] flex items-center justify-between shrink-0">
                      <span className="text-[9px] text-zinc-500 font-medium">
                        {isPousadas ? 'Hóspede está aguardando o link...' : 'Notificação de reserva enviada...'}
                      </span>
                      <button
                        type="button"
                        className={`px-3 py-1.5 ${accentBg} border ${isPousadas ? 'border-emerald-500/35 hover:bg-emerald-500/20 text-emerald-400' : 'border-blue-500/35 hover:bg-blue-500/20 text-blue-400'} font-bold rounded-lg flex items-center gap-1.5 transition-all duration-200 cursor-pointer shadow-md ${isPousadas ? 'shadow-emerald-500/5' : 'shadow-blue-500/5'} active:scale-95`}
                      >
                        <Hand className="w-3.5 h-3.5" />
                        {isPousadas ? 'Assumir Conversa' : 'Aprovar Reserva'}
                      </button>
                    </div>
                  </div>
                </div>
