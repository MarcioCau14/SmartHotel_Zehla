...
// Default brain health from global metrics
const defaultBrainHealth = {
  edge_latency: 23,
  brain_queue: 4,
  voice_swarm: 2,
  cache_hit_rate: 87.3,
...
export function SwarmOverview({ brainHealth: _brainHealthProp }: SwarmOverviewProps) {
  const [agents, setAgents] = useState<AIAgent[]>([]);
  const [loading, setLoading] = useState(true);

  // Use merged brain health — prop + defaults
  const brainHealth = { ...defaultBrainHealth, ..._brainHealthProp };

  useEffect(() => {
    fetch('/api/agents')
      .then(r => r.json())
      .then(d => {
        // Handle both wrapped { success, agents } and plain array
      ...
      <div className="glass-card p-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {[
            ...
            { label: 'Cache Hit', value: `${Number(brainHealth.cache_hit_rate).toFixed(1)}%`, ok: Number(brainHealth.cache_hit_rate) > 80 },
