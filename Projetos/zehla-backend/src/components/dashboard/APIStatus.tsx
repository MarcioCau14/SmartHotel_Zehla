    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div className="glass-card p-4">
          <Globe className="w-4 h-4 text-[#FF5500] mb-1.5" />
          <div className="text-lg font-bold text-[#efefef]">{initialProviders.length}</div>
          <div className="text-[10px] text-[#4d4d4d]">Provedores Configurados</div>
        </div>
        <div className="glass-card p-4">
        ...
        <div className="glass-card p-4">
          <Zap className="w-4 h-4 text-[#FF5500] mb-1.5" />
          <div className="text-lg font-bold text-[#efefef]">360K</div>
          <div className="text-[10px] text-[#4d4d4d]">Tokens Hoje (Total)</div>
        </div>
        <div className="glass-card p-4">
          <CheckCircle className="w-4 h-4 text-[#FF5500] mb-1.5" />
          <div className="text-lg font-bold text-[#FF5500]">99.7%</div>
          <div className="text-[10px] text-[#4d4d4d]">Sucesso Geral</div>
        </div>
      </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-[10px] text-[#4d4d4d] mt-0.5">{provider.provider}</div>
                  </div>
                </div>
                <div className="flex items-center gap-4">
                  <div className="hidden sm:flex items-center gap-4 text-[10px] text-[#4d4d4d]">
                    <span className="text-right">
                      <div className="text-[#898989]">Tokens Hoje</div>
                      <div className="font-mono text-[#b4b4b4]">{totalTokens.toLocaleString('pt-BR')}</div>
                    </span>
                    <span className="text-right">
                      <div className="text-[#898989]">Uptime</div>
              ...
              <div className="border-t border-[#2e2e2e] p-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
                  <div className="bg-white/[0.02] rounded-lg p-2.5">
                    <div className="text-[10px] text-[#4d4d4d]">Tipo</div>
                    <div className={`text-xs font-medium ${provider.type === 'gratuita' ? 'text-[#FF5500]' : 'text-[#FF5500]'}`}>
                      {provider.type === 'gratuita' ? 'Gratuita' : 'Paga'}
                    </div>
                  </div>
                  <div className="bg-white/[0.02] rounded-lg p-2.5">
                    <div className="text-[10px] text-[#4d4d4d]">Rate Limit</div>
                    <div className="text-xs font-mono text-[#b4b4b4]">{provider.rateLimit}</div>
                  </div>
                  <div className="bg-white/[0.02] rounded-lg p-2.5">
                    <div className="text-[10px] text-[#4d4d4d]">Uptime</div>
                    <div className="text-xs font-mono text-[#FF5500]">{provider.uptime}</div>
                  </div>
                  <div className="bg-white/[0.02] rounded-lg p-2.5">
                    <div className="text-[10px] text-[#4d4d4d]">Provedor</div>
                    <div className="text-xs text-[#b4b4b4]">{provider.provider}</div>
                  </div>
                </div>
                      ...
                      <div key={model.name} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02] hover:bg-white/[0.04] transition-colors">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs text-[#4d4d4d]">{cfg.label}</div>
                          </div>
                        </div>
                        <div className="flex items-center gap-4 text-xs">
                          <div className="text-right hidden sm:block">
                            <div className="text-[#4d4d4d]">Latência</div>
                            <div className={`font-mono ${model.latency_ms > 500 ? 'text-[#FF5500]' : 'text-[#b4b4b4]'}`}>
                              {model.latency_ms > 0 ? `${model.latency_ms}ms` : '—'}
                            </div>
                          </div>
                          <div className="text-right hidden sm:block">
                            <div className="text-[#4d4d4d]">Sucesso</div>
                            <div className={`font-mono ${model.success_rate >= 99 ? 'text-[#FF5500]' : model.success_rate > 0 ? 'text-[#FF5500]' : 'text-[#363636]'}`}>
                              {model.success_rate > 0 ? `${model.success_rate}%` : '—'}
                            </div>
                          </div>
                          <div className="text-right hidden md:block">
                            <div className="text-[#4d4d4d]">Tokens</div>
