...
import { 
  ...
  ExternalLink, QrCode, RefreshCw, Loader2, AlertCircle, TrendingUp
} from 'lucide-react';
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="glass-card p-4">
              ...
              <div className="text-2xl font-bold text-[#FF5500] font-mono">
                {campaigns.length > 0 ? ((campaigns.reduce((acc, c) => acc + c.repliedCount, 0) / campaigns.reduce((acc, c) => acc + c.sentCount || 1, 0)) * 100).toFixed(1) : 0}%
              </div>
            </div>
            <div className="glass-card p-4">
              ...
              <div className="text-2xl font-bold text-rose-500 font-mono">
                {campaigns.reduce((acc, c) => acc + (c.optOutCount || 0), 0)}
              </div>
            </div>
            <div className="glass-card p-4">
              <div className="text-[10px] text-[#4d4d4d] uppercase font-bold mb-1">Ganhos de Score (Brain)</div>
              <div className="text-2xl font-bold text-green-400 font-mono">
                +{campaigns.reduce((acc, c) => acc + (c.repliedCount * 30) - (campaigns.reduce((acc, c) => acc + (c.optOutCount || 0), 0) * 50), 0)} pts
              </div>
            </div>
          </div>
