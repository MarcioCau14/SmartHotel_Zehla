  ...
  return (
    <>
      ...
      >
        ...
        >
          {leads.filter(l => l.latitude && l.longitude).map(lead => {
            let icon = icons.inactive;
            if (lead.status === 'CONVERTED' || lead.funnelStage === 'CONVERTED') icon = icons.converted;
            else if (lead.status === 'TRIAL_STARTED' || lead.funnelStage === 'TRIAL') icon = icons.trial;
            else if (lead.status === 'QUALIFIED' || lead.funnelStage === 'INTERESTED' || lead.conversionScore >= 50) icon = icons.interested;
            else if (lead.funnelStage === 'AWARE' || lead.funnelStage === 'ENGAGED') icon = icons.aware;
