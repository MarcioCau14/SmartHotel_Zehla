...
export function CognitiveObservability() {
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="glass-card p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              { label: 'ZDR Uptime', value: String((security.zdr_uptime as string) || '99.97%'), ok: true },
              { label: 'Veredictos Safe', value: String(((security.guardian_verdicts as Record<string, number>).safe) || 0), ok: true },
              { label: 'HITL Pending', value: String(Array.isArray(security.hitl_pending) ? security.hitl_pending.length : 0), ok: Array.isArray(security.hitl_pending) && security.hitl_pending.length < 5 },
              { label: 'LGPD/PCI', value: security.lgpd_compliant && security.pci_compliant ? 'Compliant' : 'Non-compliant', ok: security.lgpd_compliant && security.pci_compliant },
            ].map((item, i) => (
              <div key={i} className="bg-white/[0.02] rounded-lg p-3">
                <div className="text-[10px] text-[#4d4d4d]">{item.label}</div>
                <div className={`text-sm font-mono font-bold ${item.ok ? 'text-[#FF5500]' : 'text-red-400'}`}>{item.value}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Zehla ML Brain Observability */}
      <div className="glass-card p-5">
        <h3 className="text-sm font-semibold text-[#b4b4b4] mb-4 flex items-center gap-2">
          <Brain className="w-4 h-4 text-[#FF5500]" />
          Zehla ML Brain — Model Performance
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          {[
            { label: 'Tone Alignment', value: `${mlMetrics.toneAlignmentScore}%`, ok: true },
            { label: 'IA Conversions', value: `${mlMetrics.conversionRate}%`, ok: true },
            { label: 'Model Drift', value: `${mlMetrics.modelDrift}%`, ok: mlMetrics.modelDrift < 5 },
            { label: 'RAG Vectors', value: String(mlMetrics.vectorsStored), ok: true },
            { label: 'RAG Status', value: mlMetrics.ragStatus, ok: true },
            { label: 'Last Fine-Tuning', value: mlMetrics.lastFineTuning, ok: true },
          ].map((item, i) => (
            <div key={i} className="bg-white/[0.02] rounded-lg p-3">
              <div className="text-[10px] text-[#4d4d4d]">{item.label}</div>
              <div className={`text-sm font-mono font-bold ${item.ok ? 'text-[#FF5500]' : 'text-amber-400'}`}>{item.value}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div className="space-y-1.5 terminal-text">
          {traces.map((trace) => (
            <div key={trace.id} className="flex items-center gap-3 py-1.5 px-2 rounded hover:bg-white/[0.02]">
              <span className="text-[10px] text-[#363636] font-mono w-16">{trace.timestamp}</span>
              <span className={`text-[10px] font-mono w-28 ${
                trace.service === 'ZEHLA_BRAIN' ? 'text-[#FF5500]' :
                trace.service === 'GUARDIAN' ? 'text-[#FF5500]' :
                trace.service === 'ZDR' ? 'text-[#FF5500]' :
                'text-[#898989]'
              }`}>{trace.service}</span>
              <span className="text-[10px] text-[#898989] flex-1">{trace.operation}</span>
              <span className={`text-[10px] font-mono ${
                trace.latency_ms > 2000 ? 'text-red-400' :
                trace.latency_ms > 100 ? 'text-[#FF5500]' :
                'text-[#FF5500]'
              }`}>{trace.latency_ms}ms</span>
              <span className={`w-2 h-2 rounded-full ${
                trace.status === 'success' ? 'bg-[#FF5500]' :
          ...
          {attestations.map((att) => (
            <div key={att.id} className="flex items-center justify-between p-3 rounded-lg bg-white/[0.02]">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="text-xs text-[#b4b4b4]">{att.type}</div>
                  <div className="text-[10px] text-[#4d4d4d]">{att.details}</div>
                </div>
              </div>
              <div className="flex items-center gap-3">
                <span className="text-[10px] text-[#363636] font-mono">{att.tenant}</span>
                <span className="text-[10px] px-2 py-0.5 rounded bg-[#FF5500]/10 text-[#FF5500] font-semibold">
                  {att.result}
                </span>
              </div>
            </div>
        ...
        <div className="space-y-2 terminal-text">
          {[
            ...
            <div key={i} className="flex gap-3 text-xs">
              <span className="text-[#363636] font-mono w-16">{log.time}</span>
              <span className={`w-2 h-2 rounded-full mt-1 ${
                log.level === 'info' ? 'bg-[#FF5500]' :
                log.level === 'warning' ? 'bg-amber-400' :
                'bg-red-400'
              }`} />
              <span className={
                log.level === 'info' ? 'text-[#898989]' :
                log.level === 'warning' ? 'text-[#FF5500]' :
                'text-red-400'
              }>{log.action}</span>
            </div>
          ))}
