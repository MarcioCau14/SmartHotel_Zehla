      ...
      <div className="space-y-4">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="glass-card p-5 animate-pulse">
            <div className="h-6 bg-[#242424] rounded w-1/3 mb-4" />
            <div className="h-10 bg-[#242424] rounded w-full" />
          </div>
        ))}
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      >
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-sm font-semibold text-[#efefef] mb-1">
        ...
        {message && (
          <motion.div
            ...
            className={`glass-card p-3 flex items-center gap-2 border ${
              message.type === 'success' ? 'border-orange-500/20 bg-orange-500/5' : 'border-red-500/20 bg-red-500/5'
            }`}
          >
            ...
            <span className={`text-xs flex-1 ${message.type === 'success' ? 'text-[#FF5500]' : 'text-red-400'}`}>
              {message.text}
            </span>
            <button onClick={() => setMessage(null)} className="text-[#4d4d4d] hover:text-[#b4b4b4]">
              ...
              <div className="p-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-[#efefef] flex items-center gap-2">
                        {info.name}
                        {config.isActive && (
                          <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-[#FF5500]/10 text-[#FF5500] border border-orange-500/20">
                            ATIVO
                          </span>
                        )}
                        {config.hasKey && !config.isActive && (
                          <span className="px-2 py-0.5 rounded-full text-[9px] font-bold bg-[#FF5500]/10 text-[#FF5500] border border-amber-500/20">
                            CHAVE SALVA
                          </span>
                        )}
                      </div>
                      <div className="text-[10px] text-[#4d4d4d]">
                        {info.tier} • Modelo: <span className="font-mono text-[#898989]">{config.model}</span>
                      </div>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        config.isActive ? 'bg-orange-500' : 'bg-[#2e2e2e]'
                ...
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <Key className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#363636]" />
                    <input
                      ...
                      className="w-full pl-9 pr-10 py-2.5 bg-[#242424] border border-[#363636] rounded-lg text-xs text-[#efefef] placeholder-neutral-600 focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-[#FF5500]/30 font-mono transition-all"
                    />
                  ...
                  {isEditing ? (
                    <motion.button
                      ...
                      disabled={saving === config.provider}
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-xs font-semibold rounded-lg transition-all disabled:opacity-50"
                    >
                    ...
                    <motion.button
                      ...
                      className="inline-flex items-center gap-1.5 px-4 py-2.5 bg-[#242424] hover:bg-[#2e2e2e] border border-[#363636] text-[#b4b4b4] text-xs font-medium rounded-lg transition-all"
                    >
                ...
                <div className="flex items-center gap-3 mt-3">
                  <a
                    ...
                    className="text-[10px] text-[#FF5500]/70 hover:text-[#FF5500] flex items-center gap-1 transition-colors"
                  >
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[10px] text-[#363636] hover:text-[#898989] flex items-center gap-1 transition-colors"
                  >
      ...
      {/* Info Note */}
      <div className="glass-card p-4 border border-[#2e2e2e]">
        <div className="flex items-start gap-2">
          <Info className="w-4 h-4 text-[#4d4d4d] shrink-0 mt-0.5" />
          <div className="text-[10px] text-[#4d4d4d] space-y-1">
            <p>
              <strong className="text-[#898989]">Segurança:</strong> As API keys são armazenadas com criptografia no servidor. 
              A ZEHLA recomenda usar provedores gratuitos (Z.ai SDK, Gemini Flash, Groq) para manter custos zero.
            </p>
            <p>
              <strong className="text-[#898989]">LGPD:</strong> Dados processados pela IA ficam no Brasil (Google Vertex AI - São Paulo). 
              Nenhum dado de hóspedes é usado para treinar modelos de terceiros.
            </p>
            <p>
              <strong className="text-[#898989]">Ativação:</strong> As chaves ficam prontas para ativação. O toggle acima ativa/desativa cada provedor sem risco.
            </p>
          </div>
        </div>
      </div>
