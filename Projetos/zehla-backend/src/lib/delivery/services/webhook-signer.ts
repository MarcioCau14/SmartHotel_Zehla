...
export class WebhookSigner {
  private static readonly SECRET = process.env.WEBHOOK_SECRET || 'zehla_war_machine_secret_2026';
  ...
  static verify(payload: any, timestamp: number, signature: string): boolean {
    ...
    if (Math.abs(now - timestamp) > 300000) {
      console.error('❌ [SECURITY] Webhook rejeitado: Timestamp expirado.');
      return false;
    }

    // 2. Reconstrói a assinatura esperada
    const expectedSignature = this.sign(payload, timestamp);

    // 3. Comparação Segura (Constant-time)
