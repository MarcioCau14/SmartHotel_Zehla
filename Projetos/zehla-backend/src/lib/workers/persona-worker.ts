 ...
 * Executa a mineração de DNA das pousadas estritamente de madrugada (03:00 AM).
 */

const personaQueue = new Queue('persona-learning', { 
...
export const personaWorker = new Worker('persona-learning', async (job: Job) => {
  ...
  try {
    ...
    console.error(`❌ [PERSONA WORKER] Falha no aprendizado de ${propertyId}:`, error);
    throw error;
  }
}, { 
  connection: redisWorker,
  concurrency: 1 // Processamento sequencial para poupar CPU/RAM
});
...
export async function scheduleNightlyLearning() {
  ...
  for (const job of repeatableJobs) {
    await personaQueue.removeRepeatableByKey(job.key);
  }

  // Agendar para todas as propriedades ativas
  const properties = await prisma.property.findMany({
    select: { id: true }
  });

  for (const prop of properties) {
    await personaQueue.add(
      `nightly-${prop.id}`, 
      { propertyId: prop.id },
      { 
        repeat: { pattern: '0 3 * * *' }, // Todo dia às 03:00 AM
        jobId: `persona-${prop.id}`
      }
    );
  }
