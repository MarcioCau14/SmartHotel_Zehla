/**
 * ZEHLA SMARTHOTEL — Circuit Breaker State Machine Value Object
 * Módulo: src/domain/decision/models/CircuitBreakerState.ts
 */
...
export class CircuitBreakerState {
  private constructor(
    ...
    if (consecutiveFailures < 0 || consecutiveSuccesses < 0 || halfOpenAttempts < 0) {
      throw new Error('Counters must be non-negative integers');
    }
    if (lastFailureAt < 0 || openedAt < 0) {
  ...
  recordFailure(config: CircuitBreakerConfig): CircuitBreakerState {
    const now = Date.now();
    switch (this.state) {
      case CircuitState.CLOSED:
  ...
  maybeTransitionToHalfOpen(config: CircuitBreakerConfig): CircuitBreakerState {
