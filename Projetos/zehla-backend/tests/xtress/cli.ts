...
export class XtressRunner {
  ...
  private printScenarioInfo(scenario: TestScenario): void {
    ...
    :  ${scenario.loadConfig.requestsPerSecond}`);
    }`);
    }`);
    }`);
     + "%)" : "DESATIVADO"}`);
  ...
  private printSummary(testRun: TestRun, vulnerabilities: typeof testRun.vulnerabilities): void {
    ...
    :    ${testRun.calibrations.filter(c => c.phase === 1).length}`);
    : ${testRun.calibrations.filter(c => c.phase === 2).length}`);
    : ${testRun.calibrations.filter(c => c.phase === 3).length}`);
