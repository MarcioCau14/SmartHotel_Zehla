...
import { CognitiveCopywriterService } from '../../../../src/domain/comercial/cognitive/CognitiveCopywriterService'
...
// Mock for CognitiveCopywriterService
class MockCognitiveCopywriterService extends CognitiveCopywriterService {
  public mockText = 'Mensagem de follow-up gerada pelo serviço cognitivo mockado'
  public shouldFail = false
  ...
  override async generate(input: any): Promise<Result<string, Error>> {
    if (this.shouldFail) {
      return Result.fail(new Error('LLM_GENERATION_FAILED'))
    }
    return Result.ok(this.mockText)
  }
}
...
describe('ProcessFollowUpUseCase Unit Tests', () => {
  ...
  let copywriterService: MockCognitiveCopywriterService
  ...
  beforeEach(() => {
    ...
    copywriterService = new MockCognitiveCopywriterService()
    eventPublisher = new DomainEventPublisher()
    useCase = new ProcessFollowUpUseCase(leadAdapter, messagingGateway, copywriterService, eventPublisher)
  })
  ...
  function createLeadInState(state: 'entrada' | 'primeira_interacao' | 'follow_up_1', hasPhone = true) {
    ...
    }).value as LeadScoreTarget

    interface LeadScoreTarget {
      value: LeadScore
    }
  ...
  it('should format message dynamically using CognitiveCopywriterService and transition lead to follow_up_1 on ENGAJAMENTO cadence', async () => {
    ...
    // Verify message was sent with correct format generated by the cognitive service
    ...
    expect(sent.message).toBe('Mensagem de follow-up gerada pelo serviço cognitivo mockado')
  ...
  it('should return error and fail-fast if copywriter service fails', async () => {
    createLeadInState('primeira_interacao')
    copywriterService.shouldFail = true
    ...
    const result = await useCase.execute(input)
    expect(result.isFail).toBe(true)
    expect(result.error.message).toBe('LLM_GENERATION_FAILED')
