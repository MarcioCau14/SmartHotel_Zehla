...
export const DEFAULT_LIMITS: ResourceLimits = {
  ...
  maxTotalPromptBytes: 50000,       // ~50KB
  maxLlmOutputTokens: 1024,
  maxLlmCostUsd: 0.10,              // USD 0.10 por request
  ...
  if (cb.state === 'OPEN') {
    if (now - cb.lastFailure > DEFAULT_LIMITS.circuitBreakerTimeoutMs) {
      ...
      throw new ResourceExhaustionError('CIRCUIT_OPEN', { 
        key, 
        retryAfter: Math.ceil((cb.lastFailure + DEFAULT_LIMITS.circuitBreakerTimeoutMs - now) / 1000) 
      });
    }
  }
}
...
export function truncateMessagesSecure(
  ...
  if (messages.length > limits.dbMaxRows) {
    throw new ResourceExhaustionError('ROW_LIMIT_EXCEEDED', {
      received: messages.length,
      max: limits.dbMaxRows,
    });
  }
  ...
  for (const msg of sliced) {
    ...
    if (msg.content.startsWith('data:') || msg.content.length > limits.maxCharsPerMessage * 2) {
      continue; 
    }
