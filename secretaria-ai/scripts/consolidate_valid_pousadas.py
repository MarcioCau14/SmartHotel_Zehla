...
HEADER_FILL = PatternFill(start_color="1F4E79", end_color="1F4E79", fill_type="solid") # Azul escuro real premium
...
# ==============================================================================
# PIPELINE DE CONSOLIDAÇÃO INTELIGENTE E FILTRAGEM (SOMENTE POUSADAS)
# ==============================================================================

def main():
    print("🧠 [SECRETARIA-IA] Iniciando Consolidação Global de POUSADAS VÁLIDAS por Cidades...")
    ...
    print(f"📊 Mapeados {len(xlsx_files)} arquivos validados para extração de Pousadas.")
    
    # Dicionário de pousadas válidas exclusivas
    # Chave: (Pousada, E-mail, Whatsapp) para remoção absoluta de duplicidades
    all_valid_pousadas = {}
    
    for file_path in xlsx_files:
        print(f"📂 Extraindo pousadas de: {os.path.basename(file_path)}")
        try:
            ...
            for row in range(2, sheet.max_row + 1):
                ...
                # --------------------------------------------------------------
                # FILTRO ESTRITO DE POUSADAS (EXCLUIR HOTÉIS/RESORTS/ETC.)
                # --------------------------------------------------------------
                exclude_keywords = [
                    "hotel", "resort", "hostel", "flat", "residence", "reserve", 
                    "motel", "inn", "spa", "club", "lodge"
                ]
                
                # Se contiver qualquer palavra de exclusão, ignora!
                ...
                # --------------------------------------------------------------
                # VALIDAÇÃO DE CONTATOS (Rigorosa mas realista)
                # --------------------------------------------------------------
                # E-mail: Precisa ter formato e MX ativo (sem erros)
                email_ok = email_val and email_val != "-" and email_val != "nan" and "@" in email_val
                
                # WhatsApp: Aceita Celular Válido e WhatsApp Business Fixo Ativo corporativo!
                # (já que pousadas usam muito telefone fixo com WA Business ativado!)
                ...
                # Somente adiciona se ambos os contatos cruciais estiverem validados!
                if email_ok and whats_ok:
                    lead_key = (pousada_name.lower().strip(), email_val.lower().strip(), re.sub(r"\D", "", whatsapp_val))
                    
                    # Guarda a pousada qualificada
                    all_valid_pousadas[lead_key] = {
    ...
    print(f"📊 Total de POUSADAS válidas e exclusivas extraídas: {len(all_valid_pousadas)}")
    
    # 3. Agrupar por Cidades
    pousadas_by_city = {}
    for lead in all_valid_pousadas.values():
        city_raw = lead["cidade"].strip()
        city_name = city_raw.title()
        
        if city_name not in pousadas_by_city:
            pousadas_by_city[city_name] = []
        pousadas_by_city[city_name].append(lead)
        
    print(f"🌆 Encontradas {len(pousadas_by_city)} cidades com pousadas válidas.")
    
    # 4. Criar Novo Workbook Excel Consolidado com Células Preenchidas
    ...
    # Tab Geral
    all_leads_sorted = []
    for city_name, leads in sorted(pousadas_by_city.items()):
    ...
    print(f"\n🎉 [SUCESSO-ESTRITO] Planilha com {len(all_valid_pousadas)} POUSADAS VÁLIDAS consolidada em: {OUTPUT_FILE_PATH}")
