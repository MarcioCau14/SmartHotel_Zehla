...
import { ThompsonSampler } from '../../decision/services/ZehlaRouter';
...
export class AgentRecommender {
  static recommend(
    ...
    try {
      ...
      // Extrair taxa de conversão estimada do ThompsonSampler (mean de cada Tier)
      const tier2Conversion = samplerStats[2]?.mean || 0.5;
      const tier3Conversion = samplerStats[3]?.mean || 0.3;

      // 1. WhatsApp Agent (Recepcionista Virtual - Tier 2)
      ...
      // 2. Revenue AI (Gestor de Tarifas & Ocupação - Tier 3)
      if (!hasPMS) {
        recommendations.push({
      ...
      // 3. Social Seller AI (Instagram Agent - Tier 2)
      recommendations.push({
      ...
      // Ordenar por prioridade (CRITICAL -> HIGH -> MEDIUM)
