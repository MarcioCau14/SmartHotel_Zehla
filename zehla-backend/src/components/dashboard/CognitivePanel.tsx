...
export function CognitivePanel() {
  ...
  return (
    <div className="space-y-6">
      ...
      {health && (
        <div className="glass-card p-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-medium text-[#b4b4b4]">Status do Modelo Soberano</h3>
            <span className="text-xs font-mono px-2 py-0.5 rounded bg-[#FF5500]/10 text-[#FF5500]">
              {String(health.gemma_engine_status)}
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div>
              <span className="text-[#4d4d4d]">Modelo</span>
              <div className="text-[#b4b4b4] font-mono">{String(health.sovereign_model)}</div>
            </div>
            <div>
              <span className="text-[#4d4d4d]">Tokens Hoje</span>
              <div className="text-[#b4b4b4] font-mono">{Number(health.tokens_today).toLocaleString('pt-BR')}</div>
            </div>
            <div>
              <span className="text-[#4d4d4d]">Cache Hit</span>
              <div className="text-[#FF5500] font-mono">{Number(health.cache_hit_rate).toFixed(1)}%</div>
            </div>
            <div>
              <span className="text-[#4d4d4d]">Fleet Nodes</span>
              <div className="text-[#b4b4b4] font-mono">{String(health.fleet_nodes)}</div>
            </div>
          </div>
        </div>
      )}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="text-sm font-medium text-[#efefef]">{llm.name}</span>
                      {isActive && (
                        <span className="text-[10px] px-1.5 py-0.5 rounded bg-[#FF5500]/10 text-[#FF5500]">
                          Modelo atual
                        </span>
                      )}
                    </div>
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {llm.rateLimit && (
                        <>
                          <span className="text-[10px] text-[#363636]">•</span>
                          <span className="text-[10px] text-[#4d4d4d]">{llm.rateLimit}</span>
                        </>
                      )}
                    </div>
                  </div>
      ...
      {/* Intent Stats Chart */}
      <div className="glass-card p-4">
        <h3 className="text-sm font-medium text-[#b4b4b4] mb-4">Distribuição de Intents por Fonte</h3>
        ...
        <div className="flex gap-4 mt-3 justify-center">
          {Object.entries(sourceColors).map(([key, color]) => (
            <div key={key} className="flex items-center gap-1.5 text-xs text-[#4d4d4d]">
      ...
      {/* Agent Fleet Grid */}
      <div>
        <h3 className="text-sm font-medium text-[#b4b4b4] mb-4">Fleet de Agentes</h3>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {agents.map((agent) => (
            <div key={agent.id} className="glass-card p-4 hover:bg-[#242424] transition-all duration-200">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xl">{agent.icon}</span>
                <span className={`status-dot ${agent.status}`} />
              </div>
              <div className="text-sm font-semibold text-[#efefef]">{agent.name}</div>
              <div className="text-[10px] text-[#4d4d4d] mb-3">{agent.role}</div>
              <div className="space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className="text-[#4d4d4d]">Tarefas</span>
                  <span className="text-[#b4b4b4]">{agent.tasksCompleted}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#4d4d4d]">Sucesso</span>
                  <span className={agent.successRate >= 99 ? 'text-[#FF5500]' : agent.successRate >= 97 ? 'text-[#FF5500]' : 'text-red-400'}>
                    {agent.successRate}%
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#4d4d4d]">Latência</span>
                  <span className="text-[#b4b4b4]">{agent.avgLatencyMs}ms</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-[#4d4d4d]">Uptime</span>
                  <span className="text-[#b4b4b4]">{(agent.uptimeHours / 24).toFixed(0)}d</span>
                </div>
              </div>
            </div>
          ))}
